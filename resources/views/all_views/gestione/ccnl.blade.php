...
@section('content_main')
  ...
  <div class="content-wrapper">
    ...
    <div class="content">
      <div class="container-fluid">
		...
		@include('all_views.gestione.newccnl')

		<form method='post' action="{{ route('ccnl') }}" id='frm_ccnl' name='frm_ccnl' autocomplete="off">
        ...
        <div class="row">
          <div class="col-md-12">
				...
				<table id='tbl_list_ccnl' class="display">
					<thead>
						<tr>
							<th>ID</th>
							<th>Descrizione</th>
							...
							<tr>
								...
								<td>
								 @if ($cc->dele=="1") 
									<font color='red'><del> 
								 @endif
									<span id='id_descr{{$cc->id}}' data-descr='{{ $cc->descrizione }}'>
										{{ $cc->descrizione }}
									</span>	
								 @if ($cc->dele=="1") 
									 </del></font>
			...
			<div class="row">
			    <div class="col-lg-12">
					<button type="button" class="btn btn-primary" onclick="$('#edit_elem').val('');$('#descr_contr').val('');$('#div_definition').show(150)">
						<i class="fa fa-plus-circle"></i> Nuovo CCNL
					</button>
					<div class="form-check form-switch mt-3 ml-3">
					  <input class="form-check-input" type="checkbox" id="view_dele" name="view_dele" onchange="$('#frm_ccnl').submit()" {{ $check }}>
					  <label class="form-check-label" for="view_dele">Mostra anche CCNL eliminati</label>
					</div>
				</div>
			</div>	
		</form>
 ...
 @section('content_plugin')
	...
	<script src="{{ URL::asset('/') }}dist/js/ccnl.js?ver=1.2"></script>
