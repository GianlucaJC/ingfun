...
@section('content_main')
  ...
  <div class="content-wrapper">
    ...
    <div class="content">
      <div class="container-fluid">
		<!-- form new ditte !-->	
		@include('all_views.gestione.newutente')

		<form method='post' action="{{ route('utenti') }}" id='frm_utenti' name='frm_utenti' autocomplete="off">
			<input name="_token" type="hidden" value="{{ csrf_token() }}" id='token_csrf'>

		@if($save_edit['esito']==false)
				...
				<tbody>
					@foreach($utenti as $utente)
						<tr>
						   ...
						   <td>
							@if ($utente->dele=="0") 
								<a href='#' onclick="edit_elem(this)"  data-mail={{$utente->email}} data-nominativo={{$utente->name}} data-idrole={{$utente->ruolo}}
								data-id_elem={{$utente->id}}>
									<button type="button" class="btn btn-info" alt='Edit' >
									<i class="fas fa-edit"></i></button>
								</a>
								<a href='#' onclick="dele_element({{$utente->id}})">
									<button type="submit" name='dele_ele' class="btn btn-danger"><i class="fas fa-trash"></i></button>	
								</a>
							@endif
							@if ($utente->dele=="1") 
								<a href='#'onclick="restore_element({{$utente->id}})" >
