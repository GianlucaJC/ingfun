$(document).ready( function () {
    $('#tbl_list_contatti tfoot th').each(function () {
        var title = $(this).text();
		if (title.length!=0)
			$(this).html('<input type="text" placeholder="Search ' + title + '" />');
    });	
    var table=$('#tbl_list_contatti').DataTable({
		dom: 'Bfrtip',
		buttons: [
			'excel', 'pdf'
		],		
        ...
        language: {
            ...
            zeroRecords: 'Nessun Contatto trovato',
            info: 'Pagina _PAGE_ di _PAGES_',
            infoEmpty: 'Non sono presenti Contatti',
            infoFiltered: '(Filtrati da _MAX_Contatti totali)',
        },
...
function edit_elem(id_elem) {
	...
	mail=$("#id_mail"+id_elem).data("descr")
