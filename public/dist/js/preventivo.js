...
function prepare_to_send(id_preventivo) {
	$('#modal_fatt').modal('toggle')
	$("#title_modal_fatt").html("Invia preventivo a destinatari")
	...
	$.ajax({
		...
		success: function (data) {
			...
			testo="Invia Preventivo";

			html="<button type='button' class='btn btn-primary' id='btn_send_fatt' onclick='send_email("+id_preventivo+")'>"+testo+"</button>"
...
function send_email(id_preventivo) {
	sel=false;
	arr_send = {};
	...
	if (fl_send==true) 
		send_real(arr_send[0]['id_ref'],arr_send[0]['mail'],num_elem,0,id_preventivo)

}
function send_real(id_ref,email,num_elem,num,id_preventivo) {
	html=""
	...
	$.ajax({
		...
		data: {_token: CSRF_TOKEN, id_cand:id_cand, titolo:oggetto, email:email, body_msg:body_msg,id_preventivo:id_preventivo},
		success: function (data) {
...
function edit_product(id_riga,last_ordine) {
	...
	$("#title_modal").html("Inserimento/Modifica dati riga preventivo")
