$(document).ready( function () {
    $('#tbl_list_aliquote tfoot th').each(function () {
        var title = $(this).text();
		if (title.length!=0)
			$(this).html('<input type="text" placeholder="Search ' + title + '" />');
    });	
    var table=$('#tbl_list_aliquote').DataTable({
		dom: 'Bfrtip',
		buttons: [
			'excel', 'pdf'
		],		
        ...
        language: {
            ...
            zeroRecords: 'Nessuna Aliquota trovata',
            info: 'Pagina _PAGE_ di _PAGES_',
            infoEmpty: 'Non sono presenti Aliquote',
            infoFiltered: '(Filtrati da _MAX_ Aliquote totali)',
        },
...
function new_aliquota() {
	$('#frm_aliquote1')[0].reset();	
	$('#edit_elem').val('');
	$('#div_definition').hide()
...
function edit_elem(id_elem) {
	...
	aliquota=$("#id_descr"+id_elem).data("aliquota")
