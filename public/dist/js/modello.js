$(document).ready( function () {
    $('#tbl_list_modelli tfoot th').each(function () {
        var title = $(this).text();
		if (title.length!=0)
			$(this).html('<input type="text" placeholder="Search ' + title + '" />');
    });	
    var table=$('#tbl_list_modelli').DataTable({
		dom: 'Bfrtip',
		buttons: [
			'excel', 'pdf'
		],		
        ...
        language: {
            ...
            zeroRecords: 'Nessuna Modello trovato',
            info: 'Pagina _PAGE_ di _PAGES_',
            infoEmpty: 'Non sono presenti Modelli',
            infoFiltered: '(Filtrati da _MAX_ Modelli totali)',
        },
...
function edit_elem(id_elem) {
	...
	id_marca=$("#id_descr"+id_elem).data("id_marca")
	$("#marche").val(id_marca)
